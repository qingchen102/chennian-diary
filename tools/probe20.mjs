// probe20.mjs — 阅读页跨页边界（保存后阅读视角）
import { spawn } from "node:child_process";
import { rmSync } from "node:fs";
const EDGE = "C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe";
const PORT = 9363;
const PROFILE = "D:/dairy/.edge-probe20";
...
await send("Input.insertText", { text: "阅读跨页" });
await sleep(300);
// 注入跨页内容并保存
await ev(`(() => {
...
await clickAt(c.x, c.y);
await sleep(1600);
// 阅读视图：第 1 页末尾 + 第 2 页开头
console.log("页码:", await ev(`document.querySelector('.page-num')?.textContent`));
console.log("阅读第1页末尾:", await ev(`(() => { const ps = [...document.querySelectorAll('.pcol.left .entry-html p')]; const last = ps[ps.length-1]; const pg = document.querySelector('.pcol.left .page'); const body = pg.querySelector('.page-body').getBoundingClientRect(); return JSON.stringify({lastText: last ? last.textContent.slice(0, 22) : 'none', gapLines: last ? (body.bottom - last.getBoundingClientRect().bottom) / 32 : 99}); })()`));
console.log("阅读第2页开头:", await ev(`(() => { const ps = [...document.querySelectorAll('.pcol.right .entry-html p')]; return JSON.stringify({firstText: ps.length ? ps[0].textContent.slice(0, 30) : 'none'}); })()`));
